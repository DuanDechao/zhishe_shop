...
class StoreDuobao extends ModelBasic
{
     ...
     * @return array
     */
    public static function getListAll($offset = 0,$limit = 10,$field = 'id,product_id,image,title,price,ot_price,start_time,stop_time'){
        $time = time();
        $model = self::where('is_del',0)->where('status',1)->field($field)
