...
class StoreCart extends ModelBasic
{
    ...
    public static function setCart($uid,$product_id,$cart_num = 1,$product_attr_unique = '',$type='product',$is_new = 0,$duobao_id = 0,$combination_id=0,$seckill_id = 0,$bargain_id = 0)
    {
        if($cart_num < 1) $cart_num = 1;
		if($duobao_id){
			$StoreDuobaoInfo = StoreDuobao::getValidProduct($duobao_id);
			if(!$StoreDuobaoInfo)
				return self::setErrorInfo("该产品已下架");
			$where = ['type'=>$type, 'uid'=>$uid, 'product_id'=>$product_id, 'product_attr_unique'=>$product_attr_unique, 'is_new'=>$is_new, 'is_pay'=>0, 'is_del'=>0, 'duobao_id'=>$duobao_id];
			if($cart = self::where($where)->find()){
                ...
                return self::set(compact('uid','product_id','cart_num','product_attr_unique','is_new','type','duobao_id'));
			}
    ...
    public static function getUserProductCartList($uid,$cartIds='',$status=0)
    {
        ...
        foreach ($list as $k=>$cart){
            ...
            if(!$product){
                ...
                $invalid[] = $cart;
                //商品属性不对应
            }else if(!StoreProductAttr::issetProductUnique($cart['product_id'],$cart['product_attr_unique']) && !$cart['combination_id'] && !$cart['seckill_id'] && !$cart['duobao_id']){
                $invalid[] = $cart;
                //正常商品
            }else{
