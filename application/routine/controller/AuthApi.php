...
use app\routine\model\store\StoreDuobao;
...
class AuthApi extends AuthController{
    ...
    public function rebate_apply_platforms(){
        ...
        return JsonService::successful($platforms);
    }
    /**
     * 首页
     */
	...
	public function share(){
        $share = GroupDataService::getData('share_page_info_setting')?:[];//分享图
		$data['share'] = $share;
		return JsonService::successful($data);
	} 
...
//            unset($productAttr[$k]['attr_values']);
    ...
    public function confirm_order(Request $request){
		...
		if(count($cartIdA) > 1) {
			...
			$duobaoinfo = StoreCart::where('id', $cartId)->find();
			if((int)$duobaoinfo['duobao_id'] > 0) $duobao_id=$duobaoinfo['duobao_id'];
			else $duobao_id=0;
        }
	...
	public function duobao_index(){
        //$banner = GroupDataService::getData('routine_duobao')?:[];//banner图
        $duobao = StoreDuobao::where('is_del',0)->where('status',1)->where('start_time','<',time())->where('stop_time','>',time())->where('cur_portion < total_portion')->order('sort desc')->select()->toArray();
        $data['duobao'] = $duobao;
	...
	public function duobao_detail(Request $request){
        $data = UtilService::postMore(['id'],$request);
        $id = $data['id'];
        if(!$id || !($storeInfo = StoreDuobao::getValidProduct($id))) return JsonService::fail('商品不存在或已下架!');
		$data['storeInfo'] = $storeInfo;
        return JsonService::successful($data);
	}
...
//            HookService::afterListen('store_product_set_cart_after',$res,$this->userInfo,false,StoreProductBehavior::class);
     ...
     * @return \think\response\Json
     */
    public function now_buy($productId = '',$cartNum = 1,$uniqueId = '',$duobaoId = 0, $combinationId = 0,$secKillId = 0,$bargainId = 0){
        if(!$productId || !is_numeric($productId)) return JsonService::fail('参数错误');
        if($bargainId && StoreBargainUserHelp::getSurplusPrice($bargainId,$this->userInfo['uid'])) return JsonService::fail('请先砍价');
        $res = StoreCart::setCart($this->userInfo['uid'],$productId,$cartNum,$uniqueId,'product',1,$duobaoId,$combinationId,$secKillId,$bargainId);
        if(!$res) return JsonService::fail(StoreCart::getErrorInfo());
...
//        StoreOrder::delCombination();//删除拼团未支付订单
	...
	public function get_user_nowmoney_detail_cart($first = 0, $limit = 8)
	{
		...
		foreach($list as $k => $v){
			$list[$k]['alias_cart_info'] = StoreOrder::searchUserOrderById($v['link_id']);
...
//            HookService::listen('store_product_order_reply',$group,$cartInfo,false,StoreProductBehavior::class);
    ...
    public function get_spread_list($first = 0,$limit = 20)
    {
        ...
        foreach ($list as $k=>$user){
            ...
            $list[$k]['price'] = $sumPrice == NULL ? '0.00' : $sumPrice;
        }
