...
use think\Url;
use app\admin\model\ump\StoreDuobaoAttr;
use app\admin\model\ump\StoreDuobaoAttrResult;
use app\admin\model\ump\StoreDuobao as StoreDuobaoModel;
 ...
 */
class StoreDuobao extends AuthController
{

    use CurdControllerTrait;

    protected $bindModel = DuobaolModel::class;
    ...
    {
        $this->assign('countDuobao',StoreDuobaoModel::getDuobaoCount());
        $this->assign('duobaoId',StoreDuobaoModel::getDuobaoIdAll());
        return $this->fetch();
    }
    public function save_excel(){
        ...
        StoreDuobaoModel::SaveExcel($where);
    }
    /**
     * 异步获取砍价数据
     */
    public function get_duobao_list(){
        ...
        $duobaoList = StoreDuobaoModel::systemPage($where);
        if(is_object($duobaoList['list'])) $duobaoList['list'] = $duobaoList['list']->toArray();
        $data = $duobaoList['list']['data'];
        foreach ($data as $k=>$v){
            $data[$k]['_stop_time'] =$v['stop_time'] ?  date('Y/m/d H:i:s',$v['stop_time']) : '';
        }
        return Json::successlayui(['count'=>$duobaoList['list']['total'],'data'=>$data]);
    }

    public function get_duobao_id(){
        return Json::successlayui(StoreDuobaoModel::getDuobaoIdAll());
    }
    /**
     * 添加夺宝产品
     * @return form-builder
    ...
    public function create()
    {
        ...
        $f[] = Form::input('info','夺宝活动简介')->type('textarea');
        ...
        $f[] = Form::number('price','人次单价')->min(0)->col(12);
        $f[] = Form::number('ot_price','原价')->min(0)->col(12);
		$f[] = Form::number('total_terms','总期数')->min(0)->precision(0)->col(12);
		$f[] = Form::number('total_portion','总份数')->min(0)->precision(0)->col(12);
...
//        var_dump($this->request->post());
        ...
        if($data['price'] == '' || $data['price'] < 0) return Json::fail('请输入产品人次售价');
        if($data['ot_price'] == '' || $data['ot_price'] < 0) return Json::fail('请输入产品原售价');
        $data['add_time'] = time();
        if($id){
            $product = StoreDuobaoModel::get($id);
            if(!$product) return Json::fail('数据不存在!');
            StoreDuobaoModel::edit($data,$id);
            return Json::successful('编辑成功!');
        }else{
            $res = StoreDuobaoModel::set($data);
			StoreDuobaoModel::refreshDuobao($res->id);
    ...
    public function duobao($id){
        ...
        $f[] = Form::input('info','夺宝活动简介',$product->getData('store_info'))->type('textarea');
        $f[] = Form::input('unit_name','单位',$product->getData('unit_name'))->placeholder('个、位');
        $f[] = Form::hidden('description',$product->getData('description'));
        $f[] = Form::dateTimeRange('section_time','活动时间');
    ...
    {
        $res = Upload::image('file','store/duobao/'.date('Ymd'));
    ...
    public function edit($id)
    {
        ...
        $f[] = Form::number('price','人次单价',$product->getData('price'))->min(0)->col(12);
		$f[] = Form::number('total_terms','总期数', $product->getData('total_terms'))->min(0)->precision(0)->col(12);
		$f[] = Form::number('total_portion','总份数', $product->getData('total_portion'))->min(0)->precision(0)->col(12);
        $f[] = Form::number('sort','排序',$product->getData('sort'))->col(12);
        $f[] = Form::number('postage','邮费',$product->getData('postage'))->min(0)->col(12);
        $f[] = Form::radio('is_postage','是否包邮',$product->getData('is_postage'))->options([['label'=>'是','value'=>1],['label'=>'否','value'=>0]])->col(12);
    ...
    public function delete($id)
    {
        if(!$id) return $this->failed('数据不存在');
        $data['is_del'] = 1;
        if(!StoreDuobaoModel::edit($data,$id))
            return Json::fail(StoreDuobaoModel::getErrorInfo('删除失败,请稍候再试!'));
        else
    ...
    public function edit_content($id){
        if(!$id) return $this->failed('数据不存在');
        $seckill = StoreDuobaoModel::get($id);
        if(!$seckill) return Json::fail('数据不存在!');
        $this->assign([
            'content'=>StoreDuobaoModel::where('id',$id)->value('description'),
            'field'=>'description',
    ...
    public function change_field(Request $request,$id,$field){
        ...
        $res = StoreDuobaoModel::edit($data,$id);
        if($res)
            return Json::successful('添加成功');
        else
    ...
    {
        if(!$id) return $this->failed('数据不存在!');
        $result = StoreDuobaoAttrResult::getResult($id);
        $image = StoreDuobaoModel::where('id',$id)->value('image');
    ...
    public function set_attr($id)
    {
        ...
        $res = StoreDuobaoAttr::createProductAttr($attr,$detail,$id);
        if($res)
            return $this->successful('编辑属性成功!');
        else
            return $this->failed(StoreDuobaoAttr::getErrorInfo());
    }
    ...
    {
        if(!$id) return $this->failed('产品不存在!');
        if(false !== StoreDuobaoAttr::clearProductAttr($id) && false !== StoreDuobaoAttrResult::clearResult($id))
            return $this->successful('清空产品属性成功!');
        else
            return $this->failed(StoreDuobaoAttr::getErrorInfo('清空产品属性失败!'));
    }
     ...
     * @param int $id
     */
    public function set_duobao_status($status,$id = 0){
        if(!$id) return Json::fail('参数错误');
        $res = StoreDuobaoModel::edit(['status'=>$status],$id);
