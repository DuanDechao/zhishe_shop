...
use app\admin\model\ump\StoreDuobaoCode;
 ...
 */
class StoreDuobao extends ModelBasic
{
    use ModelTrait;
...
//        $model = $model->join('StoreProduct p','p.id=s.product_id');
     ...
     * @return array
     */
    public static function getDuobaoIdAll(){
     ...
     * @return int|string
     */
    public static function getDuobaoCount(){
        return self::where('is_del',0)->count();
    }

	public static function refreshDuobao($duobaoId){
		$info = self::where('id', $duobaoId)->field('cur_term, total_terms, cur_portion, total_portion')->select()->toArray();
		if(count($info) != 1 || $info[0]['cur_term'] >= $info[0]['total_terms'] || ($info[0]['cur_term'] > 0 && $info[0]['cur_portion'] < $info[0]['total_portion'])){
			return false;
		}
		if($info[0]['cur_term'] < $info[0]['total_terms']){
			$res = StoreDuobaoCode::initDuobaoCodes($duobaoId, $info[0]['cur_term'] + 1, $info[0]['total_portion']);
			$res = $res && self::edit([
            'cur_term'=>$info[0]['cur_term'] + 1,
            'cur_portion'=> 0,
			],$duobaoId,'id');
		}
		return $res;
